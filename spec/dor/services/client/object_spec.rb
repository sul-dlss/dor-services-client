...
RSpec.describe Dor::Services::Client::Object do
  ...
  describe '#indexable' do
    subject(:request) { client.indexable(cocina: cocina) }

    let(:cocina) { build(:dro) }
    let(:expected_solr) do
      { 'id' => cocina.externalIdentifier }
    end

    before do
      stub_request(:post, 'https://dor-services.example.com/v1/objects/druid:bc123df4567/indexable')
        .with(
          body: cocina.to_json,
          headers: {
            'Content-Type' => 'application/json'
